        ...
        with tf.GradientTape() as tape:
            ...
            if train_set.epoch_completed % 1 == 0 and train_set.epoch_completed not in logged:
                ...
                if (train_set.epoch_completed + 1) % 2 == 0:
                    print(
                        '----epoch:{}, whole_loss:{}, contrast_loss:{},clf_loss:{},gen_loss:{}, c_index:{},auc:{},precision_test:{},recall_test:{},f_score_test:{},accuracy_test:{}, '
...
#                                                                                 l2_regularization))
#     # discriminator = DISCRIMINATOR(hidden_size=hidden_size, feature_dims=28)
#     encoder = Encoder(hidden_size=32, model_type='TimeLSTM1')
...
#                 mask_input_x_train[random_select_list_sort_index, mask_index, :] = 0
#             # 生成预测的序列
#             trajectory_encode_last_h, trajectory_encode_h_list = encoder((mask_input_x_train,input_day_train_), batch=batch_size)
#             # context_state = mlp(trajectory_encode_last_h)
...
#                 tf.keras.losses.mse(input_x_train_[:, mask_index, :], predicted_trajectory_x_train[:, mask_index, :]))
#
#             real_decode_h, real_trajectory_encode_h_list = encoder((input_x_train_,input_day_train_ ), batch=batch_size)
#             clf_loss = 0
#             neg_likelihood_loss = 0
#             for v in range(visit_len):
#                 predicted_output = sap(real_trajectory_encode_h_list[:, v, :])
...
#             # shuffled_input_x_train = input_x_train_[:, shuffle_index, :]
#
#             shuffled_generated_decode_h, shuffled_generated_decode_h_list = encoder((shuffled_input_x_train, input_day_train_[:,shuffle_index ]),
#                                                                                     batch=batch_size)
#
...
#                 batch_test = input_x_test.shape[0]
#                 context_state_test, real_trajectory_encode_h_list_test = encoder((input_x_test,input_day_test), batch=batch_test)
#                 predicted_output_test_list = np.zeros((batch_test, 0))
#                 for v in range(visit_len):
#                     predicted_output_test = sap(real_trajectory_encode_h_list_test[:, v, :]).numpy()
#                     predicted_output_test_list = np.concatenate([predicted_output_test_list, predicted_output_test],
...
def train_and_save():
    ...
    reapet = 10
    model_type = 'train_model_mimic_timelstm3'
    ...
    for i in range(reapet):
        ...
        if i % 5 == 0 and i > 0:
            ...
            re_save.to_excel(
                '{}/sh_rate={}_mask_RATE={}_{}_{}_hidden_size={}_learning_rate={}_l2_regularization={}_reapet{}times.xlsx'.format(
    ...
    result_all.to_excel(
        '{}/sh_rate={}_mask_RATE={}_{}_{}_hidden_size={}_learning_rate={}_l2_regularization={}_reapet{}times5_v3.xlsx'.format(RESULT_SAVE_DIR,SHUFFLE_RATE, MASK_RATE,
                                                                                                   model_type,
                                                                                                   time_windows,
                                                                                                   hidden_size,
                                                                                                   learning_rate,
