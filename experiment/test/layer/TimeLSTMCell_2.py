...
class TimeLSTMCell_2(Layer):
    def __init__(self,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(TimeLSTMCell_2, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        # w_x
        self.kernel = self.add_weight(shape=(input_shape[0][1], self.units * 6),
                                      name='kernel',
                                      initializer=self.recurrent_initializer,
                                      regularizer=self.recurrent_regularizer,
                                      constraint=self.recurrent_constraint)
        ...
        self.kernel_t_1 = self.kernel[:, self.units*4:self.units*5]
        self.kernel_t_2 = self.kernel[:, self.units*5:]
        ...
        if self.use_bias:
            ...
            self.bias_t_1 = self.bias[:, self.units*4:self.units*5]
            self.bias_t_2 = self.bias[:, self.units*5:]
        else:
            self.bias_i = None
            self.bias_f = None
            self.bias_c = None
    ...
    def call(self, inputs, states, training=None):
        ...
        c_m_ = f*c_tm1 + i*self.activation(x_c + K.dot(h_tm1_c, self.recurrent_kernal_c))*t_1_constraint
        c_m = f*c_tm1 + i*self.activation(x_c + K.dot(h_tm1_c, self.recurrent_kernal_c))*t_2
